...
(function() {
    ...
    (function() {
        ...
        function getUpdatedName() {
            ...
            // Se não encontrou nome, tentar buscar do localStorage
            if (!nameToUse || nameToUse.length === 0) {
                try {
                    ...
                    if (profileData) {
                        ...
                        if (parsed.full_name) {
                            nameToUse = parsed.full_name.split(' ')[0].trim();
                        } else if (parsed.username) {
                            nameToUse = parsed.username.replace(/^@+/, '').trim();
                        }
                    }
                } catch (e) {
                    // Ignorar erro
            ...
            return nameToUse || 'ele(a)';
        }

        // Salvar HTML original mas atualizar o nome antes de usar
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // Buscar nome atualizado antes de iniciar a animação
                    const updatedName = getUpdatedName();
                    // Substituir o nome no HTML original antes de digitar
                    let htmlToType = originalHTML.replace(/André/g, updatedName);
                    // Também substituir qualquer outro username-display que possa estar no HTML
                    htmlToType = htmlToType.replace(/<span class="username-display">[^<]*<\/span>/g, `<span class="username-display">${updatedName}</span>`);
                    typeText(toolTitle, htmlToType, 45);
                    observer.disconnect();
                }
            });
        }, { threshold: 0.3 });
    ...
    (function() {
        ...
        const fullName = profileUsernameElement.textContent.trim();
            ...
            // Se ainda não encontrou nome, usar fallback
            if (!nameToUse || nameToUse.length === 0) {
                nameToUse = null;
            }
